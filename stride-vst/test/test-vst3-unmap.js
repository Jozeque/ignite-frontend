...
ok('unmapParamByTouch defined + guards on unlearn mode', /void\s+StrideWrapperProcessor::unmapParamByTouch[\s\S]{0,80}if\s*\(!\s*unlearnMode\.load\(\)\)\s*return/.test(procC));
