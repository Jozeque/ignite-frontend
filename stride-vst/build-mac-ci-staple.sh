...
# Sanity-check the bundled StrideInject payload — without it, "Install to
# Ableton" can't install the inject Remote Script and direct inject is dead.
if [ ! -f "$DIST/Stride/Stride.app/Contents/Resources/StrideInject/__init__.py" ]; then
    echo "❌ Bundled StrideInject payload missing from Stride.app"
    exit 1
fi
echo "      ✅ Bundled StrideInject payload verified"
...
2. Launch Stride from /Applications. Signed with an Apple Developer
   ID and notarized by Apple - no Gatekeeper bypass, no right-click
   Open, no xattr. Just double-click.
