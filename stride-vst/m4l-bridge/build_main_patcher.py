...
def tbtn(bid, x, y, w, h, text, prect, fontsize=9.0):
    """A face button. parameter_enable 0 on purpose: this is a command, never an
    automatable Live parameter (an enabled one would also claim a slot in the
    device's parameter list and show up in Live's automation lanes)."""
    box(bid, "textbutton", x, y, w, h,
        numinlets=1, numoutlets=3, outlettype=["", "", "int"],
        parameter_enable=0, presentation=1, presentation_rect=prect,
        fontname="Outfit", fontsize=fontsize, rounded=3.0, text=text,
        bgcolor=[0.141, 0.102, 0.063, 1.0],
        bgovercolor=[0.196, 0.141, 0.086, 1.0],
        textcolor=ORANGE500, textovercolor=[0.99, 0.60, 0.30, 1.0],
        outlinecolor=[0.776, 0.443, 0.169, 1.0])
...
# INJECT TO CLIP + TAKE BACK. The user picks a MIDI clip in Live, then presses
# INJECT: the lanes they drew in the VST are written into that clip as automation,
# 1:1, and Stride hands those knobs to Live. TAKE BACK reverses it for every printed
# lane at once. Both sit on the DEVICE, not in the VST, because picking the clip and
# pressing the button are one gesture in the same window.
#
# HEIGHT IS FIXED. Live's device chain gives a Max device ~169px and clips the rest,
# which is why the ground panel is exactly 90x169. Two buttons therefore cost the
# dedicated result line: the result shares the ACTIVE/STANDBY line instead and
# reverts to it after INJECT_FACE_HOLD_MS. One line that says what is happening
# reads better than two lines that are usually both idle.
#
#   status/result  112..128      INJECT  131..150      TAKE BACK  151..170
# The result is written to obj-face-st, the SAME comment that carries
# ACTIVE/STANDBY: node sends "injected set <text>" and the readout reverts to the
# status line on its own. No second comment, so nothing overlaps the buttons.
tbtn("obj-face-inject", 300, 70, 90, 22, "INJECT", [8.0, 131.0, 74.0, 18.0])
msg("obj-inject-msg", 300, 100, 55, "inject")
wire("obj-face-inject", 0, "obj-inject-msg", 0)

tbtn("obj-face-take", 300, 130, 90, 22, "TAKE BACK", [8.0, 150.0, 74.0, 17.0], fontsize=8.0)
msg("obj-take-msg", 300, 160, 60, "takeback")
wire("obj-face-take", 0, "obj-take-msg", 0)
...
obj("obj-6", 20, 110, 210, "route voice probe status injected", 1, 5, ["", "", "", "", ""])
wire("obj-3", 0, "obj-6", 0)
# face readout: node sends "status set ACTIVE"
wire("obj-6", 2, "obj-face-st", 0)
wire("obj-6", 3, "obj-face-st", 0)           # inject result shares the ACTIVE/STANDBY line
wire("obj-inject-msg", 0, "obj-3", 0)        # INJECT button -> node.script ("inject" handler)
wire("obj-take-msg", 0, "obj-3", 0)          # TAKE BACK button -> node.script ("takeback" handler)
...
box("obj-10", "comment", 20, 200 + (NUM // 4) * 210, 900, 33,
    numinlets=1, numoutlets=0,
    text="Generated by build_main_patcher.py, which also writes StrideBridge.amxd. Never hand-edit either one: re-run the script. bridge-server.js, bridge_max.js, rasterizer.js, log-scaling.js, inject-writer.js and node_modules must travel next to the .amxd.")
...
# ── and the shipping device ───────────────────────────────────────────────
# An .amxd is an `ampf` chunk container whose `ptch` payload is exactly the
# .maxpat bytes plus a CR LF NUL terminator - verified byte for byte against the
# device Max itself saved (2026-08-31). Writing it here means a face change is a
# code change: no Max round trip, and the .amxd-drift check in
# test/test-vst3-bridge.js stays green in CI. Max still opens and re-saves the
# file normally, and a re-save is byte-identical to this.
#
# Container layout (all lengths little-endian uint32):
#   'ampf' 4 'aaaa'   'meta' 4 1   'ptch' <size> <json + CR LF NUL>
payload = open("StrideBridge.maxpat", "rb").read() + b"\r\n\x00"
header = (b"ampf" + (4).to_bytes(4, "little") + b"aaaa"
          + b"meta" + (4).to_bytes(4, "little") + (1).to_bytes(4, "little")
          + b"ptch" + len(payload).to_bytes(4, "little"))
with open("StrideBridge.amxd", "wb") as f:
    f.write(header + payload)

print("StrideBridge.maxpat: %d boxes, %d lines (voices inlined)" % (len(boxes), len(lines)))
print("StrideBridge.amxd:   %d bytes (ampf container, ptch payload %d)" % (len(header) + len(payload), len(payload)))
